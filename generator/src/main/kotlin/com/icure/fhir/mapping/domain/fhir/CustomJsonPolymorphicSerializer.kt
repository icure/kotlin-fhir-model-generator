package com.icure.fhir.mapping.domain.fhir
...
abstract class CustomJsonPolymorphicSerializer<T : Any>(
	...
	override val descriptor: SerialDescriptor = buildClassSerialDescriptor(tSerialName) {
        element(discriminatorField, String.Companion.serializer().descriptor)
    }
	...
	override fun deserialize(decoder: Decoder): T {
		...
		val json = decoder.decodeJsonElement() as? JsonObject
            ?: throw SerializationException("Expected to decode a JSON object")
		val serialNameElement = json.jsonObject[discriminatorField] ?: throw SerializationException("Field '$discriminatorField' not found")
		val serialName = (serialNameElement as? JsonPrimitive)?.takeIf { it.isString }?.content ?: throw SerializationException(
            "Field '$discriminatorField' is not a string"
        )
	...
	override fun serialize(encoder: Encoder, value: T) {
		...
		val json = encoder.json.encodeToJsonElement(serializer, value) as? JsonObject
            ?: throw SerializationException("Expected to encode a JSON object for ${serializer.descriptor.serialName}")
